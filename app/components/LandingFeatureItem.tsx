  ...
  return (
    <div className="feature-item-transition mx-auto mb-[120px] grid max-w-[1200px] grid-cols-1 items-center gap-20 lg:grid-cols-2">
      <div className={`feature-content ${isReversed ? 'lg:order-2' : 'lg:order-1'}`}>
        <div className="mb-5 inline-flex h-10 w-10 items-center justify-center rounded-full bg-[#2563eb] text-lg font-semibold text-white">
          {number}
        </div>
        <h3 className="mb-5 text-[32px] font-semibold text-[#1a1a1a]">
          {title}
        </h3>
        <p className="mb-6 text-[15px] font-normal leading-[1.7] text-[#1a1a1a]">
          {description}
        </p>
        <div className="flex flex-wrap gap-2.5">
          {tags.map((tag, index) => (
            <span
              key={index}
              className="rounded-[20px] bg-[#eff6ff] px-4 py-2 text-[13px] font-medium text-[#2563eb]"
            >
              {tag}
            </span>
          ))}
